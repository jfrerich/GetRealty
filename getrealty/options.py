...
# parser.add_argument('--rnumbers ALL', help='pull All rnumbers in the specified -county cache')
    parser.add_argument('--county', help='County (choices are Bastrop, Williamson)', required=True)
    ...
    parser.add_argument('--minvalue', help='Min Assessed Value (Advanced Search)')
    parser.add_argument('--maxvalue', help='Max Assessed Value (Advanced Search)')
    parser.add_argument('--o', '--output', help='output name of .XLSX and search files')
    parser.add_argument('--date', help='append date to output of .XLSX')
    parser.add_argument('-no_query_ping_server', action="store_true", help='don\'t query user if want to submit server ping')
    parser.add_argument('-no_query_overwrite_xls', action="store_true", help='don\'t query user if want to overwrite xls')
    parser.add_argument('--run_dir', help='Location of cache, searches, and excel dir. (if defined, all exist in run_dir, otw, includes /\$county/ for each dir')
    parser.add_argument('-use_last_adv_search', action="store_true", help='use last advanced search request don\'t send to server')
    parser.add_argument('-sep_wkts', action="store_true", help='If > 900 entires, place 900 entries in separate workbooks')
    parser.add_argument('-force',action="store_true", help='force server retreiving of Ommitted Rnumbers from DEFAULT_MIN_VALUE > value > DEFAULT_MAX_VALUE')
    parser.add_argument('--work_dir')


    group_db_query = parser.add_argument_group('DB QUERY OPTIONS')
    group_db_query.add_argument('-db_search', action="store_true")
    group_db_query.add_argument('--password', help="WHERE command for searching DB. Ex. WHERE r_num=R53761")
    ...
    group_new_db_options.add_argument('--update_db_file_junk', help='''.xls file with desired changes
            to make to DB''')
    # group_new_db_options.add_argument(' ', help='.xls file with desired changes parser.parse_args')

    # group_new_db_options.add_argument(' ', help='-sql_search <comma separated list of sql column options>')
            #
...
#
    ...
    if config['defaults']['COUNTY'] is 'Bastrop':
        ...
        config['defaults']['HOST_TAX_ACCESSOR_ADVANCED'] = 'http://www.bastroptac.com'
        config['defaults']['HOST_TAX_ACCESSOR_WWW_ADVANCED'] = 'www.bastroptac.com'

        config['defaults']['GIS_WWW'] = 'http://gis.bisconsultants.com/bastropcad/?slayer=0&exprnum=0&esearch="'
        ...
        config['defaults']['HOST_TAX_ACCESSOR_ADVANCED'] = 'http://search.wcad.org'
        config['defaults']['HOST_TAX_ACCESSOR_WWW_ADVANCED'] = 'search.wcad.org'

        config['defaults']['GIS_WWW'] = 'http://gisapp.wcad.org/?pin='
# 	else {
# 		$logger->error("ERROR: Must provide a valid county");
# 		exit;
# 	}
...
if __name__ == '__main__':
    ...
    args = generate_argparser()
    print(args)
    # print(args.no_query_overswrite_xls)
    # parser.add_argument('-no_query_overwrite_xls', action="store_true", help='don\'t query user if want to overwrite xls')
    # generate_argparser.
