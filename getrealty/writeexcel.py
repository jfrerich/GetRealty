...
def writeExcel(rnumbers):
    ...
    rnumber_arrays = []
	# build arrays of rnumbers arrays with max in each of from defined DEFAULT value
...
def writeWorkBook(workbook,num_rnumbers,rnumbers_only):
    ...
    worksheet_rnum = workbook.add_worksheet(config['defaults_static']['RNUM_NOTES_SHEET'])
    ...
    (row,col) = writeMergedHeaders(workbook,worksheet,worksheet_db,row,col,printhash)

    col=0

	# Write the headings
	# get non-merged headers
    (row,col) = writeNonMergedHeaders(workbook,worksheet,worksheet_db,row,col,printhash)
    # exit()
	...
	# 	while (my @vals = $it->()) {
	# 		($row,$col) = writeRnumberRows($workbook,$worksheet,$worksheet_db,$row,$col,$printhash,@vals);
	# 		($row_rnum_sheet,$col_rnum_sheet) = writeRnumberSheet($workbook,$worksheet_rnum,$row_rnum_sheet,$col_rnum_sheet,$num_rnumbers,$printhash,@vals);
    else:
        (row,col) = writeRnumberRows(workbook,worksheet,worksheet_db,row,col,printhash,rnumbers_only)
    #     ($row_rnum_sheet,$col_rnum_sheet) = writeRnumberSheet($workbook,$worksheet_rnum,$row_rnum_sheet,$col_rnum_sheet,$num_rnumbers,$printhash,@rnumbers_only);
    ...
    worksheet.autofilter(1, 0, num_rnumbers + 1, num_headers + 1) # set autofilter
    worksheet_db.autofilter(1, 0, num_rnumbers + 1, num_headers + 1); # set autofilter
...
def writeRnumberRows(workbook,worksheet,worksheet_db,row,col,printhash,rnumbers_only):
	...
	# columns from the printArray. Before got them from db, but couldn't reorder columns easily
    ...
    for rnumber in hash_values['rnumbers']:
        ...
        for heading in headingsArray:
            ...
            if heading is "rpg":
                NOTES_value = hash_values['rnum_values'][rnumber]['NOTES_rnum_sheet']
            ...
            if heading in printhash['format']:
                ...
                format_new = setFormatting(workbook,value,my_format,format_val)
                # format_new = setFormatting(workbook,value,my_format,format_val)
            else:
                format_new = ""
            ...
            if heading in printhash['cond_format']:
                ...
                setConditionalFormatting(workbook,worksheet,cond_format,col_letter,row_p1)
            ...
            if heading in printhash['type']:
                ...
                if my_type is "file":
                    ...
                    if value is not "":
                        ...
                        value = "n/a";
                        worksheet.write(row, col, value, getWkbFormats(workbook, "format_orange"))
                        worksheet_db.write(row, col, value, getWkbFormats(workbook, "format_orange"))
                        col += 1
                        continue
                elif my_type is "gis":
                    if value:
                        value = getGoogleLink(value)
                        label = "GIS"
                elif re.match("link:(\S+)",my_type):
                    m = re.match("link:(\S+)",my_type)
                    label = m.group(1)
                ...
                # print("2. {}, {}, value = {},  heading = {}".format(col, row,
                                                                  # value, heading))
...
def setConditionalFormatting(workbook,worksheet,cond_format,col_letter,row_p1):
    ...
    if cond_format is "format1":
        ...
        writeCondText(worksheet,col_letter,row_p1,"begins with","1"    ,getWkbFormats(workbook, "format_grn")     )
        writeCondCell(worksheet,col_letter,row_p1,"="          ,"0"    ,getWkbFormats(workbook, "format_red")     )
        writeCondText(worksheet,col_letter,row_p1,"begins with","1 np" ,getWkbFormats(workbook, "format_grn")     )
        writeCondText(worksheet,col_letter,row_p1,"begins with","0 no" ,getWkbFormats(workbook, "format_red")     )
        writeCondText(worksheet,col_letter,row_p1,"begins with","n/a"  ,getWkbFormats(workbook, "format_orange")  )
        ...
        worksheet.conditional_formatting( cell,
			{
                ...
                'criteria': "<",
                'value': "5",
                'format': getWkbFormats(workbook,"format_grn"),
            }
        )
        worksheet.conditional_formatting( cell,
            {
                ...
                'minimum'  : "5",
                'maximum'  : "20",
                'format'   : getWkbFormats(workbook,"format_orange"),
            }
        )
        worksheet.conditional_formatting( cell,
            {
                ...
                'criteria' : ">",
                'value'    : "20",
                'format'   : getWkbFormats(workbook,"format_red"),
            }
        )
    else:
        pass
...
def writeNonMergedHeaders(workbook,worksheet,worksheet_db,row,col,printhash):
    ...
    for heading in headingsMergedArray:
        ...
        worksheet.write(row, col, heading_new, getWkbFormats(workbook, "format_non_merged"));
        worksheet_db.write(row, col, heading_new, getWkbFormats(workbook, "format_non_merged"));
