...
def readResponseData(page_type,rnumber,response_file_suffix):
    ...
    if page_type == "History":
        readRnumHistPageResponseData(rnumber, response_file_suffix, response_file)
    elif page_type == "Details":
        readRnumDetailPageResponseData(rnumber, response_file_suffix, response_file)
    elif page_type == "Bills":
        readRnumBillsPageResponseData(rnumber, response_file_suffix, response_file)
    elif page_type == "Data":
        response_file = cache_dir + '/' + rnumber + '/' + config['defaults_static']['DATASHEET_PAGE_XML']
        readRnumDataSheetPageResponseData(rnumber, response_file_suffix, response_file)
...
def readRnumHistPageResponseData(rnumber, hash_key, response_file):
    ...
    hash_global['DBWriteValues'][rnumber][hash_key].update({'MinTaxYear':tags.get('mintaxyear')})
    ...
    for t in tags:
        ...
        if 'TaxYear' not in hash_global['DBWriteValues'][rnumber][hash_key]:
            hash_global['DBWriteValues'][rnumber][hash_key].update({'TaxYear':{}})
        if taxyear not in hash_global['DBWriteValues'][rnumber][hash_key]['TaxYear']:
            hash_global['DBWriteValues'][rnumber][hash_key]['TaxYear'].update({taxyear:{}})
        hash_global['DBWriteValues'][rnumber][hash_key]['TaxYear'][taxyear].update({name:value})
...
def readRnumDetailPageResponseData(rnumber, hash_key, response_file):
	...
	# return(0) unless checkValuesExistInResponseFile($response_file, @check_values);
    #
	# get first level mains
    # (return1,return4) = getDetailMainEntries(rnumber,hash_key,response_file)
    getDetailMainEntries(rnumber,hash_key,response_file)

	# Exemption Codes
	# my $return2 = getDetailEntityCodes($rnumber, $hash_key, $response_file);
	# my $return3 = getDetailValuesBreakdown($rnumber, $hash_key, $response_file);
    #
	# #unless ($return1 && $return2 && $return3 && $return4) {
	# unless ($return4) {
	# 	#print "DET: return1=$return1 return2=$return2 return3=$return3 return4=$return4\n";
	# 	$logger->warn("DET: return4=$return4");
	# 	return (0)
	# }
...
def getDetailPropIDPropOwnerID(rnumber,hash_key,soup):
   ...
   # @check_values = ("PropertyID", "PropertyOwnerID");
    ...
    for line in soup.get_text().split("\n"):
        # print("line=", line)
        if re.search("PropertyID=(\d+)",line):
            m = re.search("PropertyID=(\d+)",line)
            propertyID = m.group(1)
        if re.search("PropertyOwnerID=(\d+)",line):
            m = re.search("PropertyOwnerID=(\d+)",line)
    ...
    prefix = config['defaults_static']['HOST_TAX_ACCESSOR'] + "/Appraisal/PublicAccess/"
    pIDandPOID = "PropertyID=" + propertyID + "&PropertyOwnerID=" + propertyOwnerID
    ...
    html_hist += config['defaults_static']['FIRSTTAXYEAR'] + "&LastTaxYear=" + config['defaults_static']['LASTTAXYEAR']

    html_data = prefix + "PropertyDataSheet.aspx?"
    html_data += pIDandPOID + "&NodeID=11"

	#my $html_gis = "http://gis.bisconsultants.com/bastropcad/?esearch=$rnumber&slayer=0&exprnum=0";
    html_gis = config['defaults_static']['GIS_WWW'] + rnumber

    hash_global['DBWriteValues'][rnumber][hash_key].update({'bills_link': html_bills})
    hash_global['DBWriteValues'][rnumber][hash_key].update({'details_link'   : html_details})
    hash_global['DBWriteValues'][rnumber][hash_key].update({'hist_link'      : html_hist})
    hash_global['DBWriteValues'][rnumber][hash_key].update({'GIS_link'       : html_gis})
    hash_global['DBWriteValues'][rnumber][hash_key].update({'datasheet_link' : html_data})

    hash_global['DBWriteValues'][rnumber][hash_key].update({'propertyID'      : propertyID})
    hash_global['DBWriteValues'][rnumber][hash_key].update({'propertyOwnerID' : propertyOwnerID})
    # return(0)
...
def getRnumbersFromAdvSearchOrRnumberInput():
    ...
    if config['defaults']['RNUMBERS'] is False:
        ...
        ADVANCED_SEARCH_RESPONSE_FILE = lib.mydirs.MyDirs().advanced_search_file(".SEARCH_RESULTS.html")
        response_file = ADVANCED_SEARCH_RESPONSE_FILE
        (properties) = getRnumbersFromAdvancedSearchResponse("",response_file, properties)
...
# 			# found a cache entry
            if RNUM in hash_global['cache'] and config['defaults']['UPDATE_DB_FROM_SERVER'] == False:
                logger.info("{} - In the Cache".format(RNUM))
                propertyOwnerID,propertyID = getPropIDAndPropOwnerIDFromCachDetails(RNUM)
#
                properties.append([RNUM, propertyOwnerID, propertyID])
            else:
				# go get the rnumber page
				# if -rnumbers "ALL", this else never gets entered
# 				$logger->info("$RNUM - Not in Cache or DB, go get it");

                getPropIDSearchResult(RNUM)

                response_file = lib.mydirs.MyDirs().advanced_search_file("RNUMBER_SEARCH_RESULTS.html")
                (properties) = getRnumbersFromAdvancedSearchResponse(RNUM,response_file,properties)
...
def getPropIDSearchResult(RNUM):
    ...
    post = '/Appraisal/PublicAccess/PropertySearchResults.aspx'
    url = host + post

    values =  {"PropertySearchType" : "1",
               "dbKeyAuth"         : "Appraisal",
               "TaxYear"           : "2013",
               "PropertyID"        : RNUM,
               "NodeID"            : "11",
    ...
    headers = {'Host':         host_www,
            ...
            'Referer':         host + '/Appraisal/PublicAccess/PropertySearch.aspx?PropertySearchType=4&SelectedItem=19&PropertyID=&PropertyOwnerID=&NodeID=11',
    ...
    # advanced_search_dir = MyDirs().advanced_search_dir()
    # ADVANCED_SEARCH_RESPONSE_FILE = MyDirs().advanced_search_file()
    # PROP_ID_SEARCH_RESPONSE_FILE_ZIPPED = advanced_search_dir + "/" + config['defaults']['OUTPUT'] + "RNUMBER_SEARCH_RESULTS.html"
    PROP_ID_SEARCH_RESPONSE_FILE_UNZIPPED = lib.mydirs.MyDirs().advanced_search_file("RNUMBER_SEARCH_RESULTS.html")
...
# 	my $fh = new FileHandle;
# 	unless (open ($fh, ">$PROP_ID_SEARCH_RESPONSE_FILE_ZIPPED")) {
# 		$logger->error("ERROR: cannot open the file '$PROP_ID_SEARCH_RESPONSE_FILE_ZIPPED': $!");

# 	my $mode = 0666;
# 	chmod $mode, $PROP_ID_SEARCH_RESPONSE_FILE_ZIPPED;
#
...
#
# 	my $cmd = "gunzip -f $PROP_ID_SEARCH_RESPONSE_FILE_ZIPPED;";
# 	system($cmd);
#
# 	close $fh;
...
def getRnumbersFromAdvancedSearchResponse(rnum,response_file,properties):
    ...
    for t in tags:
        ...
        array = re.search("\((.*)\)",rest).group(1).split(',')
    ...
    if rnum != "" and len(rnumbers_only) is not 1:
        print("\n")
        print("ERROR : ", rnum, "Property ID Search returned more than one Property")
        print("        ", rnumbers_only)
    ...
    # EXAMPLE SECTION
    # <td valign="top">
    #   <label class="ssPropertyLink" onclick="ViewPropertyOrOwners( 1 , 189371 , 65680 )">
    #     R90079
    #   </label>
    # </td>
    ...
    for my_rnum in rnumbers_only:
        ...
        rnum_tag = values_bd_table.find('label', text = re.compile(my_rnum), attrs = {'class' : 'ssPropertyLink'})
        # get parent of <label> and the 3rd next sibling is the property value
        value = str(rnum_tag.parent.nextSibling.nextSibling.nextSibling.get_text()).strip()
        value = re.sub('\$','',value)
        value = re.sub(',','',value)
...
def pusharray_to_hash2(rnumber,hash_key,array):
    ...
    tmp_arr = []
    for v in array:
        v =re.sub(':$','',v)   # : after for key names
        v =re.sub('\s\+','',v) # + listed after values
        v =re.sub('\s=','',v)  # = listed after values (assessed)
        v =re.sub('(\d),(\d)',r'\1\2',v) # remove commas from values only
        v =re.sub('\$','',v)   # remove $ signs
        tmp_arr.append(v)
...
def getPost(page_type, my_property, response_file_suffix):
	...
	# $logger->info("**  SERVER: GET $page_type Page : rnum=$rnumber, poID=$PropertyOwnerID, pID=$PropertyID");
...
def getPastCurrentTotalAmntsDue(rnumber,hash_key,response_file):
	...
	# check that response file is not corrupt.
    check_values = ("tblFees", "Past Years Due", "Current Amount Due", "Total Amount Due")
# 	return(0) unless checkValuesExistInResponseFile($response_file, @check_values);
    ...
    hash_global['DBWriteValues'][rnumber][hash_key].update({"Past Years Due":value})
    hash_global['DBWriteValues'][rnumber][hash_key].update({"Current Amount Due":value2})
    hash_global['DBWriteValues'][rnumber][hash_key].update({"Total Amount Due":value3})
...
#
    ...
    for rows in table.find_all('tr')[4:]:
        ...
        if  re.search("total",year):
            hash_global['DBWriteValues'][rnumber][hash_key]['dates_paid'].update({last_year:last_date})
    ...
    hash_global['DBWriteValues'][rnumber][hash_key].update({"DateResponseCaptured":date_response})
...
def readRnumDataSheetPageResponseData(rnumber,hash_key,response_file):
    ...
    for line in lines:
        ...
        if re.search('>Rooms<\/text>',line):
            found_rooms_line = 1
        elif (found_rooms_line):

			# reached the end of the sales
            if re.search('>Page<\/text>',line):
                break

            line = re.sub('.*>(.*)<\/text>',r'\1',line) # remove commas from values only
            ...
            if re.search('\d\d\/\d\d\/\d\d\d\d',line):
                ...
                if foundLastTimeSold == 0:
                    line_arr = line.split("/")
                    line = "/".join([line_arr[2].strip(),line_arr[0],line_arr[1]])
                    hash_global['DBWriteValues'][rnumber][hash_key]['LastTimeSold'] = line
                    foundLastTimeSold = 1

    hash_global['DBWriteValues'][rnumber][hash_key]['NumTimesSold'] = numtimessold
# 	close $fh;

def createDataSheetXML(rnumber, response_file):

    command = config['defaults_static']['PDFTOHTML_SRC'] + ' -xml ' + response_file
...
def getAdvancedSearchPageResponse():
	...
	# $logger->info("** Get Advanced Search Page Response **");

    host = config['defaults_static']['HOST_TAX_ACCESSOR_ADVANCED']
    host_www = config['defaults_static']['HOST_TAX_ACCESSOR_WWW_ADVANCED']

    post = '/Appraisal/PublicAccess/PropertySearchResults.aspx'
    url = host + post

	# create the request with variables
    values = {"AbstractSubdivisionCode" : config['defaults']['SUBDIVISION'],
              "NeighborhoodCode"        : "",
              "AcresLeast"              : config['defaults']['ACRES_LEAST'],
              "AcresMost"               : config['defaults']['ACRES_MOST'],
              "Block"                   : "",
              "TractLot"                : "",
              "StreetName"              : "",
              "StreetNumberLeast"       : "",
    ...
    # }
    #
	# gunzip the resulting file
	# system("gunzip -f $response_file_zipped");
...
def getPropIDAndPropOwnerIDFromCachDetails(RNUM):
    ...
    cache_dir = lib.mydirs.MyDirs().cachedir()
    response_file = "/".join([cache_dir,RNUM,config['defaults_static']['PROP_DETAIL_RESULTS']])

    AssessedValOnNextLine = 0
    for line in open(response_file, "r"):
        # print (line)
        if re.search('PropertyID=(\d+)',line):
            my_search = re.search('PropertyID=(\d+)',line)
            propertyID = my_search.group(1)
        if re.search('PropertyOwnerID=(\d+)',line):
            propertyOwnerID = my_search.group(1)

        if re.search('Assessed:', line):
            ...
            value = re.sub('^\s+', '', value)       # remove beginning whitespace
            value = re.sub('\$', '', value)         # remove Dollar sign
            value = re.sub(',', '', value)          # remove comma
            value = re.sub('(\d+).*', '\1', value)
...
# 	}
#
# 	$hash_global['DBWriteValues']->{$rnumber}->{$hash_key}->{Entity_Codes} = \@array;
